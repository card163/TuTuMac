...
- **系统镜像管理**:顶部工具栏 → 「系统镜像管理」,列出 `sdkmanager` 已知的全部
  Android 系统镜像(默认只显示与本机 CPU 架构兼容的),按 API 等级分组,一键点「下载」
  即可安装(会自动接受许可证确认提示,不需要手动在终端里敲 y),下载过程中会实时显示
  进度条(解析 sdkmanager 输出里的百分比)。「新建实例」界面里也有跳转入口。
